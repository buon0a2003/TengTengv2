...
<body>
    ...
    <div class="info">
        ...
        <p><strong>Ngày nhập:</strong> {{ \Carbon\Carbon::parse($record['ngaynhap'])->format('d/m/Y') }}</p>
        <p><strong>Kho:</strong> {{ $record['kho'] }}</p>
        <p><strong>Nhà cung cấp:</strong> {{ $record['nhacungcap'] }}</p>
        ...
        <table>
            ...
            <tbody>
                @php
                    $tongsoluong = 0;
                @endphp
                @foreach($chitietphieunhap as $index => $item)
                    @php
